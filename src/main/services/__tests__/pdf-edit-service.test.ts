...
describe('PdfEditService', () => {
  ...
  describe('applyOperations - reorder', () => {
    ...
    it('페이지 순서를 역순으로 변경할 수 있다', async () => {
      const testPdf = await createTestPdf(4, `reorder-reverse-${Date.now()}.pdf`);

      await service.applyOperations({
        filePath: testPdf,
        operations: [{ type: 'reorder', pageIndices: [], newOrder: [3, 2, 1, 0] }],
      });
    ...
    it('newOrder 길이가 페이지 수와 다르면 에러가 발생한다', async () => {
      const testPdf = await createTestPdf(3, `reorder-invalid-${Date.now()}.pdf`);
    ...
    it('newOrder에 중복 인덱스가 있으면 에러가 발생한다', async () => {
      ...
      await expect(
        service.applyOperations({
          filePath: testPdf,
          operations: [{ type: 'reorder', pageIndices: [], newOrder: [0, 0, 1] }],
        })
      ).rejects.toThrow('New order contains duplicate indices');
    });

    it('newOrder에 유효하지 않은 인덱스가 있으면 에러가 발생한다', async () => {
      const testPdf = await createTestPdf(3, `reorder-invalid-idx-${Date.now()}.pdf`);

      await expect(
        service.applyOperations({
          filePath: testPdf,
          operations: [{ type: 'reorder', pageIndices: [], newOrder: [0, 1, 5] }],
        })
